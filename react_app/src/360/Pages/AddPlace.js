...
export default function AddPlace() {
  ...
  const handelSubmit1 = (title, description, passwordo) => {
    let postedFinal = postedImages.map((fin) => {
      ...
      // changedScale.secure_url = fin.secure_url.replace("upload/", "upload/c_limit,w_0.69/");
      return changedScale;
    });
    setShowForm(false);
    ...
    form.append("hotSpots", '[]');
    form.append("transitionEffect", JSON.stringify({type: transitionTypes[0], duration: transitionDurations[0]}));
    const config = {
      headers: {
    ...
    console.log(form);
    axios
      .post(`/api/places/add`, form, config)
    ...
    axios.post(`/api/counter/`, { uuid1 }).then((res) => {
      console.log(res.data);
    });
    axios.post(`/api/like/`, { uuid2 }).then((res) => {
      console.log(res.data);
    });
  };
                  ...
                  <Form>
                    ...
                    <FormGroup className="has-icon-left form-label-group position-relative">
                      <Input
                        ...
                        id="Description"
                        rows="5"
                        placeholder="360 tour descrition"

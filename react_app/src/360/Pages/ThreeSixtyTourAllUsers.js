...
export default function ThreeSixtyTour() {
  ...
  // const [usermailfromuser, setusermailfromuser] = useState(user.user.email);
  const handle = useFullScreenHandle();
  // const forceUpdate = useForceUpdate();
  const classes = useStyles();
  const { products, likes } = React.useContext(ProductContext);
  ...
  const dislike = () => {
    return (
      ...
      setLikeunavailable(false),
      axios
        .put(`/api/like/${idss}/decrement`)
        .then((res) => {
          console.log(res.data);
        })
    );
  ...
  const [viewport, setDefaultViewport] = useState(false);
  ...
  const getPlaces = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        console.log(img);
        qImg.url = img.secure_url.replace("upload/", `upload/q_${imageQ}/`);
        setImageURLHd(img);
        setImageURLSd(qImg);
        setImageURL(img);
        ...
        // console.log(res.data.place.uuid2);
        setInterval(() => {
          setLoading(false);
        }, 1000);
      })
  ...
  return (
    <>
      ...
      {/* {alert(`email user from product :${mailuserFromPorduct}`)}
      {alert(`email user from user context :${usermailfromuser}`)} */}
      {!loading && (
                ...
                {!loading && nadir && rotation ? (
                  <div onMouseDown={(e) => setrotation(false)}>
                    <Frame
                      ...
                      animation={`property: rotation;  to: 0 360 0; loop: ${loop}; dur: ${rotationSpeed}; dir:${direction}; easing :easeOutBack	;`}
                      zoom={zoom}
                      nadir={nadirImage}

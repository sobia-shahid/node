...
import { Card, CardBody, CardImg, Row, Badge, Col, Button, Progress, FormGroup, Label } from 'reactstrap';
...
const signupSchema = yup.object({
  email: yup.string().required('Email is required').email('Email must have proper format'),
  password: yup.string().required('Password is required').min(6, 'Too Short!').max(25, 'Too Long!'),
  username: yup.string().required('Username is required').min(5, 'Too Short!').max(10, 'Too Long!'),
  name: yup.string(),
  bio: yup.string(),
  website: yup.string(),
  twitter: yup.string(),
    ...
    if (isMember) {
      ...
      axios
        ...
        .then((res) => {
          ...
          const { token, user } = res.data;
          swal('Good job!', 'Welcome to Walking, Please Login in and enjoy your time!', 'success', {
            button: 'Done',
            timer: 6000
          });
          const newUser1 = { token, user };
        ...
        <div className="container1">
          ...
          <div className="login">
            <div className="login-back">
              <div className="title1">{isMember ? <h1>SIGN IN</h1> : <h1>REGISTER</h1>}</div>
              <p className="register-linko">
                {isMember ? 'Need to register  ' : ' ALREDY MEMEBER'}
                <button type="button" onClick={togelMemeber1} style={{ color: '#0ca8fd', outline: 'none' }}>
                  click here
                </button>
              </p>
              <Formik
                initialValues={isMember ? { email: '', password: '' } : { email: '', password: '', username: '' }}

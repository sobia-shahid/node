...
const RESET_PASSWORD_KEY = 'accountactivatekey12345';
// client url
...
const signUp = async (req, res, next) => {
  const error = validationResult(req); //validator
  if (!error.isEmpty()) {
    return next(new HttpError('unable to sign up ,invalid input passed, please check your data ', 422));
  }
  const {
    username,
    email,
  ...
  const createdUser = new User({
    username,
    email,
    image: 'https://res.cloudinary.com/dx1zby8rs/image/upload/v1601817748/hemdgw3pqmp0cdtujlel.png',
    password,
    places,
    creator,
    name,
...
const updateUser = async (req, res, next) => {
  ...
  if (req.files) {
    ...
    User.findByIdAndUpdate(
      uid,
      { username, name, bio, website, twitter, facebook, linkid, google, instagram },
      { new: true }
    ).then((user) => {
      const { username, name, bio, website, twitter, facebook, linkid, google, instagram, email, image } = user;
...
const getUserById = async (req, res, next) => {
  ...
  try {
    ...
    const error = HttpError('can not find user with this user id, sorry Elias', 500);
    return next(error);
  }
...
const forgetpassword = async (req, res) => {
  ...
  User.findOne({ email }, (err, user) => {
    if (err || !user) {
      return res.status(400).json({ error: "user with this email does'nt exist" });
    }
    ...
    return user.updateOne({ resetlink: token }, (err, success) => {
      ...
      let mailDetails = {
        ...
        html: `<p>${CLIENT_URL}/forgot/?token=${token}</p>`
      };
...
const resetpassword = async (req, res) => {
  ...
  console.log(resetlink, password);
  ...
  } else res.status(401).send('resetlink is not available');
};

...
const fs = require('fs')
const Likes = require('./like-controllers.js')
...
const addPlace = async (req, res, next) => {
  const {
    ...
    imgsData,
    featured,
    tourHd,carouselDesing ,openCarousel,email,imageUser,tourCreator,uuid1,lastEdit,uuid2,
    hotSpots,
    transitionEffect,
  ...
  const newPlace = new Place({
    ...
    pause,
    showImageFeaturedInPause,
    playicon,disTourTitle,pauseOpacity,cssTourTitle,EnableLine,LineTitle,carouselDesing,openCarousel,email,imageUser,tourCreator,uuid1,lastEdit,uuid2,
    hotSpots,
    transitionEffect
  })
  newPlace
...
const updatePlace = async (req, res, next) => {
  ...
  const { title, description, category, city, link, long, id, imgsData,
    passwordo, havePassword, publish, useit, rotation, rotationSpeed, 
    openDescription, loop, direction, zoom, pause, showImageFeaturedInPause, 
    playicon, disTourTitle, pauseOpacity, cssTourTitle, EnableLine, LineTitle, 
    carouselDesing, tourHd, lastEdit, 
    hotSpots, 
    transitionEffect
    } = req.body
  ...
  if (req.files) {
    ...
    cloudinary.uploader
      ...
      .then(() => {
        ...
        place.tourHd=tourHd;
        place.imgsData=imgsData;
        place.lastEdit= lastEdit;
        place.hotSpots = hotSpots;
        place.transitionEffect = transitionEffect;
        if (imageUrl) {
...
const deletePlace = async (req, res, next) => {
  ...
  try {
    place = await Place.findById(placeId);
    const uuid2 = place.uuid2;
    if (uuid2) {
      Likes.deleteLike(uuid2)
    }
    await place.remove();
  } catch (err) {
